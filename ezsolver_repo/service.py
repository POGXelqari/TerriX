...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        action = payload.get("action", "enter_lobby").strip()
        ...
        with _count_lock:
            _queued_count += 1
        print(f"[service] queued — sitekey={sitekey!r} url={siteurl!r} action={action!r} "
        ...
        t0 = time.time()
        try:
            print(f"[service] solving sitekey={sitekey!r} url={siteurl!r} action={action!r} "
                  f"(active={_active_count}/{MAX_WORKERS})")
            token = solve(sitekey, siteurl, timeout=timeout, action=action, proxy=proxy)
